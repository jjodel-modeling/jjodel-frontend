...
**Fix strutturale raccomandato:** **eseguito il 2026-08-12.** Il canale è l'icona, non la tinta: il badge del menu porta ora il glifo del tipo invece della lettera. Quattro glifi su cinque sono quelli che `common/entityMeta.ts` già assegna agli stessi kind — `boxes`, `box`, `arrow-left-right`, `eye` — così il menu e il tree dicono la stessa cosa con lo stesso segno; `refactoring` non ha una voce lì e prende `arrow-repeat`, che nessun altro tipo di documento usa. Nessun asset nuovo (Bootstrap Icons è già dipendenza), nessun colore nuovo, nessun ritorno della tinta per tipo: le cinque pastiglie restano slate, com'è giusto per una famiglia. Misurato a video sul menu aperto: **cinque glifi distinti su cinque righe, un solo fondo**. Il campo `icon` è nuovo in `DocumentTypeEntry` ed è obbligatorio, così un tipo aggiunto in futuro non può entrare nel menu senza scegliersi un segno. La lettera resta l'identità del badge ovunque un tipo compaia da solo, e resta anche nel sottomenu dei metamodelli, dove è l'iniziale di un nome e non un tipo. La seconda superficie, le icone del tree, era già chiusa da R-RAIL-33, che tiene la scala fuori dal tree: là i glifi hanno forme diverse e il colore non porta più il tipo per nessuno.
**Priorità:** chiusa.
**Effort stimato:** speso — la decisione, più un campo, cinque valori e una riga di SCSS.
**Riferimenti:**
...
**Fix strutturale raccomandato:** **eseguito il 2026-08-12**, commit `6383e97e6`. I tre blocchi sono via. La rimozione ha richiesto una riga che nessuno aveva previsto: in dark le undici `background: transparent` della copia del pannello non tenevano solo la `-bg` del foglio del tree, tenevano anche la metà scura della quarta palette, che è scopata `html[data-theme="dark"]`, cioè (0,2,1), e batte il `background: transparent` di base a (0,2,0). Tolte le undici, sei kind prendevano una pastiglia traslucida: DClass `rgba(14,165,233,.2)`, DModel `rgba(100,116,139,.2)`, DPackage `rgba(245,158,11,.2)`, e così via. Ripristinato con una riga sola, `[data-theme="dark"] &:not(.tree-viewpoint):not(.tree-leaf-view) { background: transparent }`, dove il `:not()` protegge le due pastiglie ratificate dall'emendamento (2) a R-RAIL-33. Misurato su tre build: light zero pixel di differenza, dark fondi identici a prima su tutti i kind, colore del glifo dipinto invariato ovunque.
**Smoke visivo:** eseguito il 2026-08-12 **sui due temi**, con l'harness in container e non con una lettura del foglio. Su `tree-DModel`, `tree-DPackage`, `tree-DClass` e `tree-DAttribute` il fondo computa `rgba(0, 0, 0, 0)` in entrambi i temi; `tree-viewpoint` e `tree-leaf-view` conservano il proprio, `rgb(226, 234, 245)` in light e `rgb(36, 46, 61)` in dark, quindi il `:not()` fa il suo mestiere; il glifo resta monocromo ovunque, `rgb(15, 23, 42)` in light e `rgb(100, 116, 139)` in dark. **Copertura dichiarata:** i kind che lo scenario dell'harness non produce (reference, operation, enum, literal, parameter, object) e i modelli M1 non sono stati misurati. La guardia è una riga sola e non per-kind, quindi li copre per costruzione; ma la copertura per costruzione non è una misura, e questo va scritto invece che taciuto.
**Priorità:** chiusa.
...
**Stato attuale:** `frontend/src/components/forEndUser/tree.scss` dichiara **ventitré** selettori entity senza alcuno scoping, con una palette propria: ambra per i package, **cyan `#0ea5e9` per le classi**, verde per gli attributi, viola per le reference, cyan più scuro per le operation, grigio per i parameter. Il foglio è importato da `components/forEndUser/Tree.tsx:16`, cioè dal tree legacy per end user, ma i selettori sono globali e colpiscono anche il tree del rail, dove perdono per specificità (in light perdono a (0,1,0) contro (0,2,0); in dark il foglio si scopa `html[data-theme="dark"]`, sale a **(0,2,1)** e **vince** contro il `background: transparent` di base del pannello. Dal 2026-08-12 è trattenuto lì da una riga di guardia in `properties-with-tree-view.scss`, dentro il blocco `.tree-node__icon`. Quella riga è uno schermo, non una soluzione: si toglie solo dopo aver scopato questo foglio sotto la radice del componente legacy, mai prima.). Misurato: rimuovendo i tre blocchi entity del rail, `.tree-DClass` passa a `rgb(14, 165, 233)`. È il cyan che R-RAIL-30 ha escluso dalla scala perché prenotato dalla selezione. Oggi non dipinge, per la stessa ragione della voce precedente.

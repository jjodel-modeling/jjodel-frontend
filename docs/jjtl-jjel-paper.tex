...
\lstdefinelanguage{jjtl}{
    morekeywords={transformation, from, to, when, helper, alert, notify, prompt, input, let},
...
Attribute -> Attribute
    when { not isDerived or confirm('Remove derived attribute ' + name + '?') } {
    name -> name
    type -> type
}
\end{lstlisting}
...
\begin{lstlisting}[style=jjel,caption={JjLet in JjScript (planned)}]
...
\jjlet{} is implemented in three phases, each building on the previous.

\paragraph{Phase 1 --- \texttt{\$identifier} token in \jjel{} lexer.}
This is the only modification to the \jjel{} layer. The lexer is
extended to recognise \texttt{\$letter\ldots} as a
\texttt{DOLLAR\_IDENT} token (before the existing bare-\texttt{\$}
error branch). The \jjel{} parser adds \texttt{DOLLAR\_IDENT} as a
...
\paragraph{Phase 2 --- \kw{let} in JjTL.}
The \jjtl{} parser is extended to recognise
...
\paragraph{Phase 3 --- \kw{let} in \textsf{JjScript}.}
\kw{let} becomes a new command type in the \textsf{JjScript} command

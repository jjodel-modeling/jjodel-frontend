...
2. **Titolo pagina fuori dalla card** (EMENDATO 01-09, screenshot utente): riga di testata sul fondo desk, sopra la card — a sinistra «State» + sottoriga, a destra `Export` (secondario chiaro) e `+ New State` (primario), stessa riga del titolo. La card inizia dalla toolbar, che tiene SOLO filtro, segmented, indicatore colonne nascoste e `Columns` (indicatore+Columns allineati a destra). Attenzione a 10j: a 0 istanze la toolbar resta spenta e la testata (ora con Export/New) resta — ricontrolla che le condizioni di 10j (`New` duplicato dalla CTA) reggano nella nuova posizione: se `New` in testata a 0 istanze torna a duplicare la CTA del cartello, vale la stessa regola (uno solo dei due, la CTA vince) — dichiara la scelta.

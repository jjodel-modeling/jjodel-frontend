import { r as registerInstance, h, H as Host, c as createEvent, F as Fragment, g as getElement, f as forceUpdate } from './index-f7721357.js';
...
class IgcDockManagerService {
  ...
  togglePin(pane, pinBehavior = IgcPinBehavior.allPanes) {
    ...
    if (!event.defaultPrevented) {
      ...
      if (this.dockManager.maximizedPane) {
        const targetPanes = parent && parent.type === IgcDockManagerPaneType.tabGroupPane && this.dockManager.maximizedPane === parent
          ? [parent]
          : args.panes;
        this.dockManager.maximizedPane = null;
  ...
  maximizePane(pane) {
    ...
    if (targetPane.type == IgcDockManagerPaneType.contentPane)
      return;
...
const IgcDockManager = class {
  ...
  renderContentPane(pane, isFloating, isFlyout) {
    ...
    return (h("igc-content-pane-component", { key: pane.id, contentId: pane.contentId, size: pane.size, isFlyout: isFlyout, unpinnedSize: pane.unpinnedSize, disabled: pane.disabled, isSingleFloating: isSingleFloatingContentPane, onMouseMove: this.togglePaneHeaderVisibility.bind(this, pane), onMouseLeave: this.removeHoveredPane.bind(this), ref: el => {
      ...
      } }, (isFloating && pane.floatingHeaderId) ? h("slot", { name: pane.floatingHeaderId }) :
  ...
  render() {
    ...
    return (h(Host, { tabindex: "0", role: "group" }, this.renderButtonsTemplates(), this.renderUnpinnedTabArea(IgcUnpinnedLocation.left), h("div", { class: "pane-container--vertical", style: { position: this.maximizedPane ? 'absolute' : 'relative' } }, this.renderUnpinnedTabArea(IgcUnpinnedLocation.top), h("div", { ref: el => this.dockedPanesContainer = el, class: "pane-container--horizontal", style: { position: this.maximizedPane ? 'absolute' : 'relative' } }, ((_a = this.layout) === null || _a === void 0 ? void 0 : _a.rootPane) && this.renderSplitPane(this.layout.rootPane, false, false), this.renderFlyoutPane(), this.renderDockingIndicators()), this.renderUnpinnedTabArea(IgcUnpinnedLocation.bottom)), this.renderUnpinnedTabArea(IgcUnpinnedLocation.right), this.renderFloatingPanes(), this.renderContextMenu(), this.renderPaneNavigator(), h("igc-trial-watermark", { text: "Trial version of Ignite UI", style: { 'z-index': '10005' } }), this.renderDropShadow()));
  }
...
const IgcPaneHeaderComponent = class {
  ...
  render() {
    ...
    return (h(Host, { part: paneHeaderParts, exportparts: exportParts }, h("div", { part: paneHeaderContentParts, class: "header-text" }, h("slot", null)), h("div", { part: paneHeaderActionsParts, class: "header-actions", [IGNORE_DRAG]: true }, this.allowPinning &&
...
const IgcTabHeaderComponent = class {
  ...
  render() {
    ...
    return (h(Host, { role: "tab", part: parts, exportparts: exportparts, "aria-label": this.header, "aria-selected": this.selected ? 'true' : 'false', "aria-disabled": this.disabled ? 'true' : 'false', tabIndex: !this.selected || this.disabled ? -1 : 0 }, h("slot", null), this.iconName &&
      h("div", { part: tabParts, onClick: this.iconClick, onKeyDown: this.keyDown, [IGNORE_DRAG]: true }, this.iconName === 'close' ? this.renderCloseButton() : this.renderMoreOptionsButton())));
  }
  get element() { return getElement(this); }
  static get watchers() { return {
    "forcedDrag": ["forcedDragChanged"],
...
const IgcTabsComponent = class {
  ...
  render() {
    ...
    return (h(Host, { role: "tablist", style: {
        flex: `${size} 1 ${size}px`
      }, exportparts: exportParts, part: parts }, top && this.renderTabHeaders(top), h("div", { part: tabsContentParts, class: "content" }, h("slot", { onSlotchange: this.slotChanged.bind(this) })), bottom && this.renderTabHeaders(top), this.renderHiddenTabsMenu()));
  }
...
const SampleComponent = class {
  ...
  render() {
    return (h(Host, null, h("div", null, h("button", { onClick: () => this.saveLayout() }, "Save Layout"), h("button", { onClick: () => this.loadLayout() }, "Load Layout"), h("button", { onClick: () => this.setActivePane() }, "Set Active Pane"), h("button", { onClick: () => this.addPane() }, "Add Floating Pane"), h("button", { onClick: () => this.addTab() }, "Add Tab Pane"), h("button", { onClick: () => this.disableContentPane() }, "Disable Tab Pane"), h("button", { onClick: () => this.focusPane() }, "Focus Pane"), h("button", { onClick: () => this.toggleProximityDock() }, "Toggle Proximity Dock"), h("input", { id: "hideOnClose", type: "checkbox", style: { marginLeft: '20px' }, ref: el => this.hideOnCloseCheckbox = el }), h("label", { htmlFor: "hideOnClose" }, "Hide on close"), h("span", { style: { marginLeft: '20px' } }, "Hidden Panes:"), h("select", { style: { width: '150px' }, ref: el => this.hiddenPanesSelect = el }, this.hiddenPanes.map(p => {
      return (h("option", { value: p.id }, p.header));
    })), h("button", { onClick: () => this.showPane() }, "Show Pane"), h("button", { onClick: () => this.showAllPanes() }, "Show All Panes"), h("button", { onClick: () => this.toggleInBoundaries() }, "Toggle In Boundaries")), h("igc-dockmanager", { layout: this.layout, onPaneClose: this.handlePaneClose.bind(this), onPaneScroll: this.handlePaneScroll.bind(this), onPanePinnedToggle: this.handlePinnedToggle.bind(this), onActivePaneChanged: this.handleActivePaneChanged.bind(this), onPaneDragStart: this.handleDragStart.bind(this), onPaneDragOver: this.handleDragOver.bind(this), onPaneDragEnd: this.handleDragEnd.bind(this), onSplitterResizeStart: this.handleSplitterResizeStart.bind(this), onSplitterResizeEnd: this.handleSplitterResizeEnd.bind(this), onFloatingPaneResizeStart: this.handleFloatingPaneResizeStart.bind(this), onFloatingPaneResizeMove: this.handleFloatingPaneResizeMove.bind(this), onFloatingPaneResizeEnd: this.handleFloatingPaneResizeEnd.bind(this), onLayoutChange: this.handleLayoutChange.bind(this),
      // resourceStrings={this.getCustomResourceStrings()}
      ref: el => this.dockManager = el,
      // allowFloatingPanesResize={false}
      showHeaderIconOnHover: 'closeOnly' }, h("div", { slot: "header1" }, h("span", { style: { color: 'red' } }, "Solution Explorer"), h("button", null, "H")), h("div", { slot: "tabHeader1" }, h("span", { style: { color: 'orange' } }, "Solution Explorer"), h("button", null, "T")), h("div", { slot: "unpinnedHeader1" }, h("span", { style: { color: 'blue' } }, "Solution Explorer"), h("button", null, "U")), h("button", { style: { background: 'pink' }, slot: "tabHeaderCloseButton" }, "Y"), h("div", { slot: "content1", style: { width: '100%', height: '100%' } }, "Content 1"), h("div", { slot: "content2", style: { width: '100%', height: '100%' } }, h("button", null, "Tests")), h("div", { slot: "content3", style: { width: '100%', height: '100%' } }, "Content 3"), h("div", { slot: "content4" }, "Content 4"), h("div", { slot: "content5", style: { width: '100%', height: '100%' } }, "Content 5"), h("div", { slot: "content6", style: { width: '100%', height: '100%' } }, "Content 6"), h("div", { slot: "content7", style: { width: '100%', height: '100%' } }, "Content 7"), h("div", { slot: "content8" }, "Content 8"), h("div", { slot: "content9", style: { width: '100%', height: '100%' } }, "Content 9"), h("div", { slot: "content10", style: { width: '100%', height: '100%' } }, h("button", null, "Test")), h("div", { slot: "content11", style: { width: '100%', height: '100%' } }, h("input", null)), h("div", { slot: "content12", style: { width: '100%', height: '100%' } }, h("p", null, "Content 12 "), h("p", null, "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. ")), h("div", { slot: "content13", style: { width: '100%', height: '100%' } }, "Content 13"), h("div", { slot: "content14", style: { width: '100%', height: '100%' } }, "Content 14"), h("div", { slot: "toolboxHeader" }, h("span", null, "[U] Toolbox")), h("div", { slot: "teamExplorerHeader" }, h("span", null, "[U] Team Explorer")))));
  }
};
...
//# sourceMappingURL=igc-button-component_20.entry.js.map
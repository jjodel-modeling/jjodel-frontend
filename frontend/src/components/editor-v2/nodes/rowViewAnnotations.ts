 ...
 * D layer has the class for it. It does not work, measured 2026-08-28:
 *
 *   - `EcoreParser.parseDAnnotation` returns `[]` on its first line
 *     (`api/data.ts:650`), so no import has ever produced an annotation;
 *   - `DAnnotationDetail` (`LModelElement.tsx:150`) is an empty class whose
 *     whole body is `// todo` — it has no key and no value to hold;
 *   - `addAnnotation` has zero call sites, so nothing writes one either.
 *
 * Filling those in is a core change. What DOES exist and works is
 ...
 * starts doing so can tell our four keys from an annotation that arrived some
 * other way.
 *
 * ── The one thing this costs ────────────────────────────────────────────────
 *
 * An `.ecore` round trip drops these, because the parser is stubbed on the read
 * side too. That is pre-existing and not fixed here; a model carrying renderer
 * overrides loses them through an export/import cycle. Filling in
 * `parseDAnnotation` would fix it for this encoding and for real EAnnotations
 * at once, and is the right place to spend that effort.
 *
...
export type RowViewAnnotationKey = 'renderer' | 'unit' | 'min' | 'max';
...
export function annotationKeyOf(source: string | null | undefined): RowViewAnnotationKey | null {
    ...
    return key === 'renderer' || key === 'unit' || key === 'min' || key === 'max' ? key : null;
}
 ...
 * A bound that is not a finite number is DROPPED rather than kept as NaN: the
 * `progress` renderer asks for both bounds and a NaN would satisfy the check
 * while producing an unpaintable bar.
 */

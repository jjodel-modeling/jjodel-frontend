...
export const CtxMenuPathSeparator = '▒';
...
function ContextMenuC(props: CtxMenuAllProps): null | null[]{
    ...
    let label = props.label || (props as any).title; // fallback
    ...
    let fullpath = (props.path ? props.path + CtxMenuPathSeparator : '') + (label || '');
    let injectProps: CtxMenuInjectedProps = {path: fullpath, nodeid: props.nodeid, graphid: props.graphid, dataid: props.dataid, viewid: props.viewid};
    ...
    let tnv = transientProperties.node[props.nodeid].viewScores[props.viewid];
    if (!tnv) {
        Log.eDevv('ContextMenu extension component (in jsx) found empty tnv.', {nodeid: props.nodeid, props, vid:props.viewid});
        return null;
    }
    if (!tnv.contextMenu) tnv.contextMenu = [];
    tnv.contextMenu.push({...props, path: fullpath, action});
...
export function ContextualEntry(props: any, children:any) { return <ContextMenuC {...props}>{children || props.children}</ContextMenuC>; }
(ContextMenuC as any).cname = 'ContextMenuC';
(ContextMenu as any).cname = 'ContextMenu';
(ContextualEntry as any).cname = 'ContextualEntry';
